...
function setup() {
  // calcular dimensiones de hexágonos
  hexWidth = hexRadius * 2;
  hexHeight = Math.sqrt(3) * hexRadius;

  // calcular columnas y filas necesarias
  columns = Math.ceil(window.innerWidth / (hexRadius * 3));
  rows = Math.ceil(window.innerHeight / (hexHeight / 2)) + 1;

  mousePos = createVector(0, 0);

  // crear canvas y meterlo en el div #hexCanvas
  const canvas = createCanvas((columns + 1/4) * (hexRadius * 3), (rows + 1) * (hexHeight / 2));
  canvas.parent("hexCanvas"); // <--- ESTA ES LA LÍNEA CLAVE
  frameRate(60);
